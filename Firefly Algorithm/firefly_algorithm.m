...
function [XBest,FBest,NumEval]=ffa(fname,D,NP,MaxGeneration,alpha,betamin,XVmin,XVmax,y,gamma)

% NP=number of fireflies
% MaxGeneration=number of pseudo time steps
% ------------------------------------------------
% alpha=0.25;      % Randomness 0--1 (highly random)
% betamn=0.20;     % minimum value of beta         % Absorption coefficient
% ------------------------------------------------
NumEval=0;
...
function gera_grafico(grava);

% grava(k,:)=[k FBest average worst];

figure(1)
hold on
plot(grava(:,1),grava(:,2),'r-');
plot(grava(:,1),grava(:,3),'b-');
plot(grava(:,1),grava(:,4),'k-');
hold off
xlabel('Geração')
ylabel('FO')
legend('Melhor','Média','Pior');
